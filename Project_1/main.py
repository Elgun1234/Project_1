...
from Project_1.core.weather import Weather
...
def chart_output(request: Request, latitude: float = 51.5002, longitude: float = -0.120000124, city="London", options:str = "temperature_2m", limit:int=50):
    ...
    visibility_values =data['hourly']['visibility'][:limit]
    labels = ",".join(labels)
    values = str(values)[1:-1]
    return templates.TemplateResponse(
        "weather.html",
        {"request": request, "data": [data, labels, values, rain_values,visibility_values], "city": [city.title(), latitude, longitude]},
    )

@app.post("/weather_send")
async def weather_send(request: Request, city:str=Form(),rain: Optional[str] = Form(None),visibility: Optional[str] = Form(None)):
